...
6) set up the dns route in cloudflare
   ...
   #+begin_src
A <cricket IP> x.cricket.hyperbolic.xyz
A <cricket IP> *.x.cricket.hyperbolic.xyz
   #+end_src
...
7) add the domain field to hyperdos values
   ...
   cricket:
     enabled: true
     # once set, hyperdos will automatically provision a cert
     # for this domain, so make sure dns in step 6 is configured first
     domain: x.cricket.hyperbolic.xyz
   #+end_src
